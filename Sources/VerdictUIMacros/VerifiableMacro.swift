...
public struct VerifiableMacro: MemberMacro {
    ...
    public static func expansion(
        ...
        // The generated members must carry the HOST TYPE's access level, not a
        // fixed one. `VerifiableView` is a public protocol, and Swift refuses to
        // satisfy a public requirement with an internal member — so on a
        // `public` view (which is every view a library module exports) an
        // unmodified `verdictProbedContent` fails to compile with
        // "must be declared public because it matches a requirement in public
        // protocol 'VerifiableView'".
        //
        // This was invisible for four waves because every fixture in the macro
        // test target is internal, most of them nested inside an XCTestCase
        // where `public` is not even expressible. `PublicVerifiableViewTests`
        // is at file scope for exactly that reason.
        let access = declaration.verdictAccessLevelPrefix
        ...
        return [
            ...
            @ViewBuilder
            \(raw: access)func verdictProbedBody(into sink: VerdictTreeSink) -> some View {
            ...
            @ViewBuilder
            \(raw: access)var verdictProbedContent: some View {
                \(probedBody)
            }
            """
        ]
...
extension DeclGroupSyntax {
    ...
    /// The access-level keyword to put on generated members, with a trailing
    /// space, or `""` when the default is correct.
    ///
    /// Generated members must be at least as visible as the type they live in,
    /// because `VerifiableView` is a PUBLIC protocol: satisfying a public
    /// requirement with an internal member is an error, so a `public` view
    /// carrying `@Verifiable` fails to compile unless the members are public
    /// too. That is not an edge case — it is every view a library module
    /// exports.
    ///
    /// Only `public` is mirrored, and the omissions are deliberate:
    ///
    /// - `open` is meaningless on a struct member and, on a class, would
    ///   promise overridability the macro cannot support. `public` satisfies
    ///   the protocol either way.
    /// - `package` cannot be spelled here without raising this package's
    ///   language-mode floor, and it satisfies no public requirement anyway.
    /// - `private`/`fileprivate`/`internal` all leave the default correct.
    ///   Widening them would EXPORT a member the author deliberately hid, so
    ///   the safe direction is to emit nothing and let Swift's own default
    ///   apply.
    var verdictAccessLevelPrefix: String {
