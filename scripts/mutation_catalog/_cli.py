...
MUTATIONS: list[Mutation] = [
    Mutation(
        name="scenario act CLI run ignores consumer TaskLocal environment",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="let environment = CommandEnvironment.standard()\n            let code = await command.run(environment, pretty: pretty)",
        new="let environment = VerdictUIRunner.$environment.withValue(nil) { CommandEnvironment.standard() }\n            let code = await command.run(environment, pretty: pretty)",
        test="ActToolTests/testCLICommandUsesConsumerEnvironmentWithoutDemoFallback",
    ),
    Mutation(
        name="scenario act CLI shared entrypoint loses observed failure exit",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="case ExitCode.verdictFailed.rawValue: return .verdictFailed",
        new="case ExitCode.verdictFailed.rawValue: return .couldNotVerify",
        test="ActToolTests/testCLICommandUsesConsumerEnvironmentWithoutDemoFallback",
    ),
    Mutation(
        name="scenario act CLI help diagnostics use stderr",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old='if !message.isEmpty { output.writeOut(message + "\\n") }',
        new='if !message.isEmpty { output.writeError(message + "\\n") }',
        test="ActToolTests/testCLIEntryPointPreservesParserStreamsAndCodes",
    ),
    Mutation(
        name="scenario act CLI semantic refusal blames the UI",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old='        if !message.isEmpty { output.writeError(message + "\\n") }\n        return .couldNotVerify',
        new='        if !message.isEmpty { output.writeError(message + "\\n") }\n        return .verdictFailed',
        test="ActToolTests/testCLIEntryPointPreservesParserStreamsAndCodes",
    ),
    Mutation(
        name="scenario act CLI omitted from command registration",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="List.self, Render.self, Actions.self, Act.self, Verify.self, Judge.self, Baseline.self,",
        new="List.self, Render.self, Actions.self, Verify.self, Judge.self, Baseline.self,",
        test="ActToolTests/testCLIParserAcceptsEveryCanonicalVerbAndPreservesPayload",
    ),
    Mutation(
        name="scenario act CLI parser skips payload validation",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="_ = try command.validatedAction()",
        new='_ = try ActCommand(scenario: scenario, action: .init(kind: "tap", probe: probe)).validatedAction()',
        test="ActToolTests/testCLIParserRejectsMalformedActionBeforeExecution",
    ),
    Mutation(
        name="scenario act CLI permits nonfinite numeric payload",
        path="Sources/VerdictUICLICore/Commands.swift",
        old='        guard action.value?.isFinite ?? true else {\n            throw ValidationError("--value must be a finite number")\n        }\n',
        new="",
        test="ActToolTests/testCLICommandRejectsMalformedPayloadBeforeScenarioLookup",
    ),
    Mutation(
        name="scenario act CLI discards caller text payload",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="action: DaemonAction(kind: kind, probe: probe, text: text, value: value),",
        new="action: DaemonAction(kind: kind, probe: probe, text: nil, value: value),",
        test="ActToolTests/testCLIParserAcceptsEveryCanonicalVerbAndPreservesPayload",
    ),
    Mutation(
        name="scenario act CLI discards caller numeric payload",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="action: DaemonAction(kind: kind, probe: probe, text: text, value: value),",
        new="action: DaemonAction(kind: kind, probe: probe, text: text, value: nil),",
        test="ActToolTests/testCLIParserAcceptsEveryCanonicalVerbAndPreservesPayload",
    ),
    Mutation(
        name="scenario act CLI misdirects the target probe",
        path="Sources/VerdictUICLICore/CommandLineInterface.swift",
        old="action: DaemonAction(kind: kind, probe: probe, text: text, value: value),",
        new='action: DaemonAction(kind: kind, probe: "wrong-probe", text: text, value: value),',
        test="ActToolTests/testCLIParserAcceptsEveryCanonicalVerbAndPreservesPayload",
    ),
    Mutation(
        name="scenario act CLI drops requested after tree",
        path="Sources/VerdictUICLICore/Commands.swift",
        old="VerdictOutput.json(StepResultWire(step, includeTree: includeTree), pretty: pretty)",
        new="VerdictOutput.json(StepResultWire(step, includeTree: false), pretty: pretty)",
        test="ActToolTests/testCLICommandEmitsObservedPayloadForAllFourVerbs",
    ),
    Mutation(
        name="scenario act CLI ignores pretty formatting",
        path="Sources/VerdictUICLICore/Commands.swift",
        old="VerdictOutput.json(StepResultWire(step, includeTree: includeTree), pretty: pretty)",
        new="VerdictOutput.json(StepResultWire(step, includeTree: includeTree), pretty: false)",
        test="ActToolTests/testCLICommandEmitsObservedPayloadForAllFourVerbs",
    ),
    Mutation(
        name="scenario act CLI loses verdict failure exit",
        path="Sources/VerdictUICLICore/Commands.swift",
        old="return step.verdict.status == .pass ? .pass : .verdictFailed",
        new="return step.verdict.status == .pass ? .pass : .couldNotVerify",
        test="ActToolTests/testCLICommandPreservesLeanWireAndThreeValuedExits",
    ),
    Mutation(
        name="scenario act CLI replaces injected registry with demonstrations",
        path="Sources/VerdictUICLICore/Commands.swift",
        old="let step = try await environment.engine.act(\n                scenario: scenario, action: probeAction, includeTree: includeTree)",
        new="let step = try await VerdictEngine(registry: DemoScenarios.registry, baselines: .standard(root: environment.pixelArtifactRoot)).act(\n                scenario: scenario, action: probeAction, includeTree: includeTree)",
        test="ActToolTests/testCLICommandUsesConsumerEnvironmentWithoutDemoFallback",
    ),
    Mutation(
