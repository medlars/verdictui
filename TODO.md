...
- [x] (2026-09-23) Recover original founding and notification conversations from the Remio Cursor archive; private archival exports restore byte-for-byte. Shared clause-level completion contracts are published and active. Fleet UI reporting guard is published and active for fresh canonical invocations: 149 unavailable, one explicit headless exclusion; 215 canonical focused checks passed. Archive hooks are now active: 113 canonical checks pass, and an actual Claude session automatically archived its prompt and SessionEnd transcript with byte-exact restoration. Codex native edit-hook dispatch and complete consumer adoption remain open.
